...
def main():
    ...
    # 使用 MixUp 数据增强
    alpha = 1.0  # 超参数 alpha，控制 MixUp 的强度
    ...
    # 定义训练数据集
    train_dataset = ImageFolder(root='OSSHIP1/Train', transform=transform)

    # 定义测试数据集
    test_dataset = ImageFolder(root='OSSHIP1/Test', transform=transform)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if accuracy > best_acc:
            path_checkpoint = "./checkpoint3/checkpoint_{}_epoch.pkl".format(epoch)  # 定义保存路径，包含epoch编号
        ...
        path_checkpoint = "./checkpoint3/lastmodel.pkl".format(epoch)  # 定义保存路径，包含epoch编号
        torch.save(checkpoint, path_checkpoint)  # 保存字典到文件中
