...
model_styles = [
    ...
    {'color': (255 / 255.0, 0 / 255.0, 0 / 255.0), 'linestyle': '--'}  # 新添加的颜色和线条样式
]
...
for i, name in enumerate(models_name):
    ...
    checkpoint = torch.load(f"checkpoint4/{name}.pkl")
    ...
    elif model_name == 'The proposed method':
        model = resnet18_1(num_classes=num_classes)
...
for i, model in enumerate(models):
    model_name = models_name[i].split('+')[0]
    # root_dir = root_dirs[i]  # 获取对应模型的root_dir
    if model_name == 'The proposed method':
        transform = transforms.Compose([
            # transforms.Resize((224, 224)),  # 调整图像大小
            transforms.ToTensor(),  # 将图像转换为张量
        ])
    ...
    for class_idx in range(num_classes):
        fpr_model[class_idx], tpr_model[class_idx], _ = roc_curve(labels_onehot[:, class_idx],
...
for model_idx, (fpr_model, tpr_model, roc_auc_model, style) in enumerate(model_results):
    model_name_display = models_name[model_idx].split("+", 1)[0]  # 获取模型名称的第一部分
    plt.plot(fpr_model["macro"], tpr_model["macro"],
             label=f'{model_name_display},  (AUC = {roc_auc_model["macro"]:0.2f})',
